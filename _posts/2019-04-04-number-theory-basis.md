...
$$ \varphi(p^a) = p^a - p^{a - 1} = (p - 1) \times p^{a - 1} $$
...
给定一个数, 他的对数是满足如下条件的幂: 以某个正数(除1外)为底的该次幂恰好等于给定的这个整数, 即对底x和数y $$ y = x^{log_x(y)} $$
...
对某素数p(对非素数也可)的原根a, a的1到(p - 1)的各次幂恰可以产生1到(p - 1)的每个整数, 且仅一次

对任何整数b, 根据模算术的定义, b满足 $$ b \equiv r\ mod\ p,\ 0 \le r \le (p - 1) $$

因此, 对任何整数b和素数p的原根a, 有唯一的幂i使得 $$ b \equiv a^i\ mod\ p,\ 0 \le i \le (p - 1) $$, 该指数i称为以a为底(模p)的b的离散对数, 记为 $$ dlog_{a, p}(b) $$(或者记为 $$ ind_a(b) $$, 许多教科书把离散对数称为指标(index))

同样有:

* $$ dlog_{a, p}(1) = 0 $$, 因为 $$ a^0\ mod\ p = 1\ mod\ p = 1 $$
* $$ dlog_{a, p}(a) = 1 $$, 因为 $$ a^1\ mod\ p = a $$

> 此处给出一个使用非素数模 $$ n = 9 $$, 这里 $$ \varphi(n) = \varphi(3^2) = 3^2 - 3 = 6 $$, $$ a = 2 $$是一个原根, 计算a的各次幂得
> 
> $$ 2^0 = 1, 2^1 = 2, 2^2 = 4, 2^3 = 8 $$
> 
> $$ 2^4 \equiv 7\ mod\ 9, 2^5 \equiv 5\ mod\ 9, 2^6 \equiv 1\ mod\ 9 $$
> 
> 因此, 对给到你个的本原根 $$ a = 2 $$(模9)的离散对数, 有下列数表
>
对数 | 0 | 1 | 2 | 3 | 4 | 5
:-: | :-: | :-: | :-: | :-: | :-: | :-: |
数 | 1 | 2 | 4 | 8 | 7 | 5
>
> 给定一个数, 要得到其离散对数, 我们可以重新排列上表, 得到
> 
数 | 1 | 2 | 4 | 5 | 7 | 8
:-: | :-: | :-: | :-: | :-: | :-: | :-: |
对数 | 0 | 1 | 2 | 5 | 4 | 3
...
$$ x = a^{dlog_{a, p}(x)}\ mod\ p\ \ \ \ y = a^{dlog_{a, p}(y)}\ mod\ p $$

$$ xy = a^{dlog_{a, p}(xy)}\ mod\ p $$

由模乘法性质有

$$ xy\ mod\ p = [(x\ mod\ p)(y\ mod\ p)]\ mod\ p $$

$$ a^{dlog_{a, p}(xy)}\ mod\ p = [(a^{dlog_{a, p}(x)}\ mod\ p)(a^{dlog_{a, p}(y)}\ mod\ p)]\ mod\ p $$

$$ = a^{dlog_{a, p}(x) + dlog_{a, p}(y)} $$

根据欧拉定理, 对任何互素的a和n有, $$ a^{\varphi(n)} \equiv 1\ mod\ n $$

此外, 任何正整数z可以表示为 $$ z \equiv q + k\varphi(n) $$, 则 $$ a^z \equiv a^q\ mod\ n $$

将其带入前面的等式有:

$$ dlog_{a, p}(xy) \equiv [dlog_{a, p}(x) + dlog_{a, p}(y)]\ mod\ \varphi(p) $$

由此有:

$$ dlog_{a, p}(y^r) \equiv [r \times dlog_{a, p}(y)]\ mod\ \varphi(p) $$
